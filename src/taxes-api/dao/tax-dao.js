...
const {TaxCreateRequest} = require("../dto/request-dto");

const ENTITY_PREFIX = 'tax'
...
class TaxDao {
    ...
    async getById(appId, id) {
        ...
        if (response && response.length > 0) {
            return new Tax(response[0]);
        } else {
            throw new ResourceNotFoundError(`app ${appId} tax ${id} not found`);
        }
    }
    ...
    async create(request) {
        const dbRequest = new TaxCreateRequest(request);
