...
const {RateCreateRequest} = require("../dto/request-dto");

const ENTITY_PREFIX = 'rate'
...
class RateDao {
    ...
    async getById(appId, id) {
        ...
        if (response && response.length > 0) {
            return new Rate(response[0]);
        } else {
            throw new ResourceNotFoundError(`app ${appId} rate ${id} not found`);
        }
    }
    ...
    async create(request) {
        const dbRequest = new RateCreateRequest(request);
