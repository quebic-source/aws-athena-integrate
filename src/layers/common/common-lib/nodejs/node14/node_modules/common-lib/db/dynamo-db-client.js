...
class DynamoDBClient {
    ...
    async findAllByPKWithCustomIndex(pkField, pk, indexName, pageRequest=new PageRequest({})) {
        const dbPageRequest = preparePaginationDBRequest(pk, pageRequest);
        const ExpressionAttributeNames = {};
        ExpressionAttributeNames[pkAttributeKey] = pkField;
    ...
    async findByPKs(ids, projection=[]) {
        const Keys = [];
        for (let id of ids) {
            const key = {};
            key[PK_FIELD] = {'S': id};
    ...
    async create(pk, sk, request) {
        const today = this._getCurrentDate();
        request[CREATED_AT_FIELD] = today;
        request[MODIFIED_AT_FIELD] = today;
        return await this._save(pk, sk, request);
    }

    async update(pk, sk, request) {
        request[MODIFIED_AT_FIELD] = this._getCurrentDate();
        return await this._save(pk, sk, request);
    }

    async _save(pk, sk, request) {
        request[PK_FIELD] = pk;
        request[SK_FIELD] = sk;
