...
export default class PackagesApiStack {
    ...
    constructor(apiGateway: ApiGateway, lambdaLayers: LambdaLayers, table: aws.dynamodb.Table) {
        ...
        const appsApiFunc = new LambdaConstructor('packages-api', {
            handlerPath: 'packages-api',
            envVariables: envVariables,
            layers: lambdaLayers.getDefaultLayers(),
        });
        grantLambdaDynamoDBAccess("packages-api-db", appsApiFunc.role, table);
        this.apiGateway.addEndpoint(appsApiFunc.lambdaFunc, "/packages", "GET");
        this.apiGateway.addEndpoint(appsApiFunc.lambdaFunc, "/packages/{packageId}", "GET");
        this.apiGateway.addEndpoint(appsApiFunc.lambdaFunc, "/packages", "POST");
        this.apiGateway.addEndpoint(appsApiFunc.lambdaFunc, "/packages/{packageId}", "PUT");
        this.apiGateway.addEndpoint(appsApiFunc.lambdaFunc, "/packages/{packageId}", "DELETE");
    }
}