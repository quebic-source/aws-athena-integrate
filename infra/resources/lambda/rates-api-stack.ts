...
export default class RatesApiStack {
    ...
    constructor(apiGateway: ApiGateway, lambdaLayers: LambdaLayers, table: aws.dynamodb.Table) {
        ...
        const appsApiFunc = new LambdaConstructor('rates-api', {
            handlerPath: 'rates-api',
            envVariables: envVariables,
            layers: lambdaLayers.getDefaultLayers(),
        });
        grantLambdaDynamoDBAccess("rates-api-db", appsApiFunc.role, table);
        this.apiGateway.addEndpoint(appsApiFunc.lambdaFunc, "/rates", "GET");
        this.apiGateway.addEndpoint(appsApiFunc.lambdaFunc, "/rates/{rateId}", "GET");
        this.apiGateway.addEndpoint(appsApiFunc.lambdaFunc, "/rates", "POST");
        this.apiGateway.addEndpoint(appsApiFunc.lambdaFunc, "/rates/{rateId}", "PUT");
        this.apiGateway.addEndpoint(appsApiFunc.lambdaFunc, "/rates/{rateId}", "DELETE");
    }
}